...
```{r}
df_train_selected <- df_train[, c("Gender_Male", "Age", "City_Patna","Academic.Pressure", "CGPA","Study.Satisfaction", "Sleep.Duration_.Less.than.5.hours.","Sleep.Duration_.More.than.8.hours.", "Dietary.Habits_Healthy","Dietary.Habits_Moderate", "Dietary.Habits_Unhealthy","Degree_.Class.12.", "Have.you.ever.had.suicidal.thoughts.._No","Have.you.ever.had.suicidal.thoughts.._Yes", "Work.Study.Hours","Financial.Stress", "Family.History.of.Mental.Illness_No", "Depression")]
df_test_selected  <- df_test[, c("Gender_Male", "Age", "City_Patna","Academic.Pressure", "CGPA","Study.Satisfaction", "Sleep.Duration_.Less.than.5.hours.","Sleep.Duration_.More.than.8.hours.", "Dietary.Habits_Healthy","Dietary.Habits_Moderate", "Dietary.Habits_Unhealthy","Degree_.Class.12.", "Have.you.ever.had.suicidal.thoughts.._No","Have.you.ever.had.suicidal.thoughts.._Yes", "Work.Study.Hours","Financial.Stress", "Family.History.of.Mental.Illness_No", "Depression")]
```
...
print(wss)
```
La variable `wss` representa la suma total de la variància interna dins dels clústers per diferents valors de k (nombre de clústers), des de 1 fins a 10.

Observem que la disminució del WSS és significativa des de `k = 1` fins a `k = 5`, moment en què passa de `975765.0 a 831195.2`. Aquesta reducció indica que afegir clústers en aquest interval millora notablement la cohesió interna dels grups.

A partir de `k = 5`, les reduccions successives són més petites (per exemple, de 831195.2 a 815267.1 i després a 806315.9), la qual cosa indica que afegir més clústers no aporta una millora significativa en la compactació dels grups.

Aquest patró mostra que la major part de la millora en la cohesió dels clústers s’aconsegueix amb 5 clústers.

Per tant, el mètode del colze recomana seleccionar `k = 5`, ja que és el punt a partir del qual la reducció del WSS comença a ser menys pronunciada i augmentar el nombre de clústers no aporta beneficis rellevants al model.

### 4.2.4 Aplicació de k-means amb k = 5  
...
A partir de l’anàlisi prèvia mitjançant el mètode del colze, hem determinat que el nombre òptim de clústers per segmentar les dades és `k=5`.
...
Els **clústers 2, 4 i 5** presenten valors més alts en pressió acadèmica (3.43, 3.37 i 3.46), estrès financer (3.40, 3.35 i 3.38) i depressió mitjana (0.81, 0.77 i 0.79 respectivament). Aquest patró suggereix que aquests factors podrien contribuir significativament a un **major malestar emocional** en aquests grups.

En canvi, els **clústers 1 i 3** reflecteixen situacions més favorables, amb nivells més baixos tant d’estrès financer (2.74 i 2.71) com de depressió (0.23 i 0.22), així com una satisfacció amb els estudis lleugerament superior (3.11 i 3.10). Aquests grups poden representar estudiants amb un **millor estat de benestar psicològic** i una càrrega acadèmica i financera més manejable.
...
Com s’ha observat prèviament, els **clústers 2, 4 i 5** són els que concentren el **nombre més elevat d’individus amb nivells alts de depressió**, fet que coincideix amb uns valors mitjans més alts de **pressió acadèmica** i **estrès financer** en aquests grups. 
...
Una observació destacada és la variable **“Have you ever had suicidal thoughts”**. Els **clústers 2, 4 i 5**, que ja mostraven valors mitjans més alts de **depressió**, **estrès financer** i **pressió acadèmica**, són també els que concentren una **proporció més elevada d’individus que han afirmat haver tingut pensaments suïcides**. Aquesta coincidència apunta cap a un perfil de **major vulnerabilitat psicològica** en aquests grups.

D’altra banda, els **clústers 1 i 3** presenten proporcions més altes d’estudiants que **no han experimentat pensaments suïcides**, en línia amb els seus valors més baixos de depressió i estrès. Aquests clústers podrien representar estudiants amb més **resiliència emocional** o **entorns menys adversos**.

Pel que fa a la variable **“Sleep Duration”**, es detecta que els clústers amb més casos de depressió (**2, 4 i 5**) presenten també una **major proporció d’individus que dormen menys de 5 hores per nit**, i menys que dormin més de 8 hores. Aquesta dada suggereix una possible **relació entre privació de son i malestar emocional**.
